...
def T_matrix_1q(U1q):
    ''' T_out = 1/2 * tr[U P_in U' P_out]
    '''
    T_out = np.zeros((4,4))
    for w in it.product(range(4),repeat=2):
        [w_in, w_out] = w
        T_out[w_out,w_in] = np.real(1./2*np.trace(np.dot(evol_U(
            U1q,Pauli_list[w_in]),Pauli_list[w_out])))
    return T_out
...
def T_matrix_2q(U2q):
    ...
    for w in it.product(range(4),repeat=4):
        ...
        T_out[4*w1_out+w2_out][4*w1_in+w2_in] = np.real(1./4.*np.trace(
            np.dot(evol_U(U2q,Pauli_in),Pauli_out)))
    return T_out
...
def T_matrix_3q(U3q):
    ...
    for w in it.product(range(4),repeat=6):
        ...
        Pauli_in = np.kron(np.kron(Pauli_list[w1_in],Pauli_list[w2_in]),
                           Pauli_list[w3_in])
        Pauli_out = np.kron(np.kron(Pauli_list[w1_out],Pauli_list[w2_out]),
                            Pauli_list[w3_out])
        T_out[16*w1_out+4*w2_out+w3_out][16*w1_in+4*w2_in+w3_in] = np.real(
            1./8*np.trace(np.dot(evol_U(U3q,Pauli_in),Pauli_out)))
    return T_out
...
def get_prob_Pauli_2q(circuit):
    ...
    gate_N_2q_list, gate_S_2q_list, gate_P_2q_list = get_NSP_sequence(
        gate_T_2q_list)
    ...
    prob_Pauli_output = {
        'state_T_list': state_T_list, 'gate_T_list': gate_T_2q_list,
        'gate_P_list': gate_P_2q_list, 'gate_S_list': gate_S_2q_list,
        'gate_N_list': gate_N_2q_list, 'index_list': index_list,
        'meas_T_list': meas_T_list, 'neg_gate': neg_gate
        }
...
def get_prob_Pauli_3q(circuit):
    ...
    index_list = circuit['index_list']
    meas_T_list, neg_meas = get_meas_T_list(circuit['meas_list'])
    gate_N_3q_list, gate_S_3q_list, gate_P_3q_list = get_NSP_sequence(
        gate_T_3q_list)
    ...
    prob_Pauli_output = {
        ...
        'gate_P_list': gate_P_3q_list, 'gate_S_list': gate_S_3q_list,
        'gate_N_list': gate_N_3q_list, 'index_list': index_list,
        'meas_T_list': meas_T_list, 'neg_gate': neg_gate
        }
    return prob_Pauli_output
...
def get_meas_RT_list(meas_T_list, x_list):
    ...
    for meas_T in meas_T_list:
        R = Rot_matrix(x_list[meas_index][0],x_list[meas_index][1],
                       x_list[meas_index][2])
...
def sample_circuit_2q(prob_Pauli, sample_size=int(1e5)):
    ...
    # gate_T_2q_list = prob_Pauli['gate_T_list']
    gate_P_2q_list = prob_Pauli['gate_P_list']
    gate_S_2q_list = prob_Pauli['gate_S_list']
    # gate_N_2q_list = prob_Pauli['gate_N_list']
    ...
    def sample_itr_2q():
        ...
        for index in range(len(gate_P_2q_list)):
            ...
            w1_temp_out, w2_temp_out, s = update_2q(w1_temp_in,
                    w2_temp_in,s,gate_P_2q_list[index],gate_S_2q_list[index])
...
#         print(s)
    ...
    print('p_estimate:',np.mean(out_list),'\t(sampling time:',
          time.time()-t,')')
...
#     gate_N_3q_list = prob_Pauli_output['gate_N_list'] ### Not used
    ...
    def sample_itr_3q():
        ...
        for index in range(len(gate_P_3q_list)):
            ...
            w1_temp_out, w2_temp_out, w3_temp_out, s = update_3q(w1_temp_in,
                w2_temp_in,w3_temp_in,s,gate_P_3q_list[index],
                gate_S_3q_list[index])
            w_list[w_index_1] = w1_temp_out
            w_list[w_index_2] = w2_temp_out
...
def sample_circuit_2q_all(prob_Pauli,sample_size = int(1e5)):
    ...
    def sample_itr_2q_all():
        ...
        for index in range(len(gate_P_2q_list)):
            ...
            w1_temp_in = w_list[w_index_1]
            w2_temp_in = w_list[w_index_2]
            w1_temp_out, w2_temp_out, s = update_2q(w1_temp_in,w2_temp_in,
                                s,gate_P_2q_list[index],gate_S_2q_list[index])
            w_list[w_index_1] = w1_temp_out
            w_list[w_index_2] = w2_temp_out
...
#         meas_T_target = np.array([1.0, 0.0, 0.0, -1.0])
    ...
    print('p_estimate:',np.mean(out_list,axis=0),'\t(sampling time:',
          time.time()-t,')')
...
def opt_Pauli_2q(prob_Pauli_output,**kwargs):
    ...
    gate_N_2q_opt_list, gate_S_2q_opt_list, \
        gate_P_2q_opt_list = get_NSP_sequence(gate_T_2q_opt_list)
    ...
    # print('Log negativity (meas):',np.log(neg_meas))
    # print('Log negativity:',np.log(neg_gate) + np.log(neg_meas),
    #       '\t(optimization time:)', time.time() -t,')')

    prob_Pauli_output_opt = {
        'state_T_list': state_T_list, 'gate_T_list': gate_T_2q_opt_list,
        'gate_P_list': gate_P_2q_opt_list, 'gate_S_list': gate_S_2q_opt_list,
        'gate_N_list': gate_N_2q_opt_list, 'index_list': index_list,
        'meas_T_list': meas_RT_list, 'neg_gate': neg_gate
        }
...
def opt_Pauli_3q(prob_Pauli_output,**kwargs):
    ...
    meas_RT_list, neg_meas = get_meas_RT_list(meas_T_list,x_list)
    gate_N_3q_opt_list, gate_S_3q_opt_list, \
        gate_P_3q_opt_list = get_NSP_sequence(gate_T_3q_opt_list)
    ...
    prob_Pauli_output_opt = {
        'state_T_list': state_T_list, 'gate_T_list': gate_T_3q_opt_list,
        'gate_P_list': gate_P_3q_opt_list, 'gate_S_list': gate_S_3q_opt_list,
        'gate_N_list': gate_N_3q_opt_list, 'index_list': index_list,
        'meas_T_list': meas_RT_list, 'neg_gate': neg_gate
        }
    return prob_Pauli_output_opt
...
def opt_neg_1q(T1q,x_in,**kwargs):
    ...
    optimize_result, dt = optimizer(cost_function, x0, options['opt_method'],
                                    niter = options['niter'])
...
def opt_Pauli_2q_global(prob_Pauli_output,**kwargs):
    ''' #### Not working ###
    '''
...
#         random_circuit, compress2q_circuit, compress3q_circuit,
#         string_to_circuit, show_connectivity, solve_qubit_circuit,
#         random_connected_circuit_2q3q)
...
#         sample_circuit_2q, sample_circuit_3q, opt_Pauli_2q, opt_Pauli_3q,
#         opt_Pauli_2q_global)
...
#     prob_Pauli_output_2q = get_prob_Pauli_2q(circuit_compress_2q)
#     print("prob_Pauli_output_2q['neg_gate']:", np.log(prob_Pauli_output_2q['neg_gate']))
#     print("-----------------2q compression with optimization-----------------")
#     prob_Pauli_output_opt_2q = opt_Pauli_2q(prob_Pauli_output_2q)
#     print("prob_Pauli_output_opt_2q['neg_gate']:", np.log(prob_Pauli_output_opt_2q['neg_gate']))
...
#     print("-----------------2q compression with optimization-----------------")
#     sample_out_opt_2q = sample_circuit_2q(prob_Pauli_output_opt_2q,
#                                           sample_size = sample_size)
#     print("---------------3q compression without optimization----------------")
#     sample_out_3q = sample_circuit_3q(prob_Pauli_output_3q,
#                                       sample_size = sample_size)
#     print("-----------------3q compression with optimization-----------------")
#     sample_out_opt_3q = sample_circuit_3q(prob_Pauli_output_opt_3q,
#                                           sample_size = sample_size)
#     print("===============================================================")
