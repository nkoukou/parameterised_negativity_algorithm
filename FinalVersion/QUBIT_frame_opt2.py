...
# x_index[gate_index] = [[x1_in_idx,x2_in_idx,...],[x1_out_idx,x2_out_idx,...]]
...
def get_connected_index(x_circuit,target_circuit_index):
    ...
    [target_state_index,target_gate_index,target_meas_index] = target_circuit_index

    idx_connect = []

    for i in target_state_index:
        for j in target_gate_index:
            idx_connect = idx_connect + np.ndarray.tolist(np.intersect1d([i],x_index_gate[j][0]).flatten())
    
    for i in target_gate_index:
        for j in target_gate_index:
            if i != j:
                idx_connect = idx_connect + np.ndarray.tolist(np.intersect1d(x_index_gate[i][1],x_index_gate[j][0]).flatten())
                
    for i in target_gate_index:
        for j in target_meas_index:
            idx_connect = idx_connect + np.ndarray.tolist(np.intersect1d(x_index_gate[i][1],x_index_meas[j]).flatten())
...
def get_target_circuit_block(x_circuit,x_target_index):
    ...
    for idx in range(len(x_index_gate)):
        if set(np.array(x_index_gate[idx]).flatten()).intersection(x_target_index):
            target_gate_index.append(idx)
    for idx in range(qubit_num):
        if (x_index_meas[idx] in x_target_index):
            target_meas_index.append(idx)
        
    target_circuit_index = [target_state_index,target_gate_index,target_meas_index]
...
def neg_gate_max(W_gate, gate, par_list_in, par_list_out):
    if W_gate==W_test:
        n = len(par_list_in)
        neg = np.abs(W_gate(gate, par_list_in, par_list_out)).sum(axis=tuple(np.arange(2*n,4*n))).max()
    else:
        neg = np.abs(W_gate(gate, par_list_in, par_list_out)).sum(axis=0).max()
    return neg
...
def get_negativity_circuit(W,circuit,x_circuit):
    target_circuit_index = [np.arange(len(circuit['state_list'])),np.arange(len(circuit['gate_list'])),np.arange(len(circuit['meas_list']))]
    return get_negativity_block(W,circuit,x_circuit,target_circuit_index)

def opt_negativity_block(W,circuit,x_circuit,target_circuit_index,niter=3,show_log=False):
    [x_list, x_index_gate, x_index_meas] = x_circuit
    ...
    def cost_function(x):
        ...
        return get_negativity_block(W,circuit,x_replaced_circuit,target_circuit_index)
    ...
    if len(idx_connect)==0:
        ...
        optimise_result = basinhopping(func, x_ref_list, minimizer_kwargs={"method":"L-BFGS-B", "jac":True}, niter=niter)
...
def random_circuit_opt(W,circuit,x_circuit,l_state=2,l_gate=5,l_meas=2,niter=3,show_log=False):
    target_state_index = np.random.choice(np.arange(len(circuit['state_list'])),l_state,replace=False)
    target_gate_index = np.random.choice(np.arange(len(circuit['gate_list'])),l_gate,replace=False)
    target_meas_index = np.random.choice(np.arange(len(circuit['meas_list'])),l_meas,replace=False)
    
    target_circuit_index = [target_state_index,target_gate_index,target_meas_index]
    idx_connect = get_connected_index(x_circuit,target_circuit_index)

    get_negativity_block(W,circuit,x_circuit,target_circuit_index)
    x_circuit_out = opt_negativity_block(W,circuit,x_circuit,target_circuit_index,niter=niter)
...
#     for x_init_idx in range(len(x_circuit[0])):
        x_target_index = x_range[l*x_init_idx:np.min([l*(x_init_idx + 1),len(x_circuit[0])])]
#         x_target_index = x_range[x_init_idx:np.min([x_init_idx + l,len(x_circuit[0])])]
        target_circuit_index = get_target_circuit_block(x_circuit_out,x_target_index)
        ...
        x_circuit_out = opt_negativity_block(W,circuit,x_circuit_out,target_circuit_index,niter=niter)
        neg_out = get_negativity_circuit(W,circuit,x_circuit_out)
        if show_log == True:    
            print('Optimized log-negativity:\t', np.log(neg_out))
...
def sequential_para_opt(W,circuit,x_circuit,l=3,niter=3,show_log=False):
    ...
    x_circuit_out = x_circuit
    for x_init_idx in range(len(x_circuit[0])):
        x_target_index = np.arange(x_init_idx,np.min([x_init_idx + l,len(x_circuit[0])]))
        target_circuit_index = get_target_circuit_block(x_circuit_out,x_target_index)
        idx_connect = get_connected_index(x_circuit_out,target_circuit_index)
...
#     test_circuit={'state_list':[np.eye(2),np.eye(2),np.eye(2),np.eye(2)],'index_list':[[0,1,2],[1,2,3],[0,1,2]],'meas_list':[np.eye(2),np.eye(2),np.eye(2),np.eye(2)]}
...
if __name__== "__main__":
    ...
    L = 30
    n = 2
    circuit_haar = haar_random_connected_circuit(N, L, n, d=2,given_state=None, given_meas=1, method='c')
    ...
    circuit_compressed = haar_circuit_compressed.copy()
    x_out,neg_list1_n3 = sequential_para_opt(W,circuit_compressed,x_circuit,l=1,niter=3)
    print('Optimized log-negativity (n=3, l=1):',np.log(neg_list1_n3[-1]))
    
    circuit_compressed = haar_circuit_compressed.copy()
    x_out,neg_list2_n3 = sequential_para_opt(W,circuit_compressed,x_circuit,l=2,niter=3)
    print('Optimized log-negativity (n=3, l=2):',np.log(neg_list2_n3[-1]))
    
    circuit_compressed = haar_circuit_compressed.copy()
    x_out,neg_list3_n3 = sequential_para_opt(W,circuit_compressed,x_circuit,l=3,niter=3)
