...
def T1q(state, x, meas=False):
    c = 1. if meas else 1./2
    T_out = np.zeros(4)
    for w in it.product(range(4),repeat=1):
        [w1] = w
        T_out[w1] = np.real(c*np.trace(np.dot(state,Pauli_list[w1])))
    T_out = np.dot(Rot_matrix(x[0],x[1],x[2]), T_out)
    return T_out

def T2q(state, x, meas=False):
    c = 1. if meas else 1./2
    T_out = np.zeros(16)
    for w in it.product(range(4),repeat=2):
        [w1, w2] = w
        Pauli = np.kron(Pauli_list[w1], Pauli_list[w2])
        T_out[4*w1+w2] = np.real(c*np.trace(np.dot(state, Pauli)))
    R1 = Rot_matrix(x[0],x[1],x[2])
    R2 = Rot_matrix(x[3],x[4],x[5])
    T_out = np.dot(np.kron(R1,R2), T_out)
    return T_out

def T3q(state, x, meas=False):
    c = 1. if meas else 1./2
    T_out = np.zeros(64)
    for w in it.product(range(4),repeat=3):
        [w1, w2, w3] = w
        Pauli = np.kron(Pauli_list[w1], Pauli_list[w2], Pauli_list[w3])
        T_out[16*w1+4*w2+w3] = np.real(c*np.trace(np.dot(state, Pauli)))
    R1 = Rot_matrix(x[0],x[1],x[2])
    R2 = Rot_matrix(x[3],x[4],x[5])
    R3 = Rot_matrix(x[6],x[7],x[8])
    T_out = np.dot(np.kron(np.kron(R1,R2), R3), T_out)
    return T_out
