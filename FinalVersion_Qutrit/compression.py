...
def compress_circuit(circuit, n, DIM):
    '''
    ...
    for k in range(len(disjoint_index_list)):
        ...
        compressed_gate_list.append(aligned_gate(disjoint_gate,disjoint_index,
                                             match_m_index(disjoint_index,n), DIM))
...
def compress_subroutine(disjoint_index_list_in, disjoint_gate_list_in,
                        ...
                        target_index_in, target_gate_in, d_loc_in, n, DIM):
    if d_loc_in == -1-len(disjoint_index_list_in):
        ...
        if m_len == t_len + d_len: ### disjointed index
            ...
            compressed_gate_list_out.append(aligned_gate(disjoint_gate,
                disjoint_index,match_m_index(disjoint_index, n),DIM))
            target_index_out = target_index_in
            target_gate_out = target_gate_in
            d_loc_out = d_loc_in
        elif m_len <= n: ###
            ...
            target_gate_out = merge_gate(disjoint_gate, target_gate_in,
                                disjoint_index, target_index_in, merged_index, DIM)
            d_loc_out = d_loc_in
        else:
...
def aligned_gate(gate, index, target_index, DIM):
    ...
    if len(index)!=len(target_index):
        ...
        for i in list(set(target_index).difference(index)):
            index_dup.append(i)
            added_dim += 1
        gate = np.kron(gate, np.eye(DIM**added_dim))
    else:
        index_dup = index
    ...
    P_matrix = permutation_matrix(list(range(len(index_dup))),
                                  new_target_index, [DIM]*len(target_index))
...
def merge_gate(U1, U2, index1, index2, m_index, DIM):
    U1_match = aligned_gate(U1,index1,m_index,DIM)
    U2_match = aligned_gate(U2,index2,m_index,DIM)
